...
export async function getFeaturedInfo(topic: string, lang: string = 'es'): Promise<{ title: string, content: string, source_urls?: string[] } | null> {
  const apiKey = typeof process !== 'undefined' && process.env ? process.env.GEMINI_API_KEY : undefined;
  if (!apiKey) return null;
  ...
  const prompt = `
    Busca información actualizada en internet sobre el siguiente tema relacionado con el mundo Halal: "${topic}".
    Proporciona un resumen informativo, útil y veraz en ${targetLang}.
    El resumen debe tener entre 2 y 3 párrafos.
    Responde SOLO en formato JSON.
    
    Esquema:
    {
      "title": "título informativo del tema",
      "content": "resumen detallado en ${targetLang}",
      "source_urls": ["lista de URLs de las fuentes consultadas"]
    }
  `;
  ...
  try {
    ...
    console.error("Gemini featured info error:", error);
    return null;
  }
}
